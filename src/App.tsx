...
export default function App() {
  ...
  useEffect(() => {
    const cachedCart = localStorage.getItem('nn_cart');
    if (cachedCart) {
      try {
    ...
    const cachedBookings = localStorage.getItem('nn_bookings');
    if (cachedBookings) {
      try {
  ...
  const saveCart = (newCart: CartItem[]) => {
    setCart(newCart);
    localStorage.setItem('nn_cart', JSON.stringify(newCart));
  };
  ...
  const handleAddBooking = (newBooking: TableBooking) => {
    ...
    localStorage.setItem('nn_bookings', JSON.stringify(updated));
  };
